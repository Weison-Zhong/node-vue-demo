module.exports = app => {
    ...
    //web端Home组件newsCats里面首先是一个数组，里面包含了多个分类，每一个分类里面有一个newsList表示多篇新闻
    //所以主体应该是分类，关联新闻去把他调出来。
    //1、先找出顶级的分类
    //下面这种会有个问题，有时候可能会查不出来东西，如果我们需要每个分类下面必须调用5条
    //那这种就实现不了，因为他是根据数量去算出来的（比如4个分类共20条数据，如果最近添加的
    //文章不均匀，全是一个分类的文章，那么查出来的就不是想要的效果了。所以这里不能简单的用populate
    ...
    //     res.send(parent)
    // })
    //要用另外一种方式，聚合查询,可以同时执行多次查询得到想要的结果
    router.get('/news/list', async (req, res) => {
        //1、先找出顶级分类
        const parent = await Category.findOne({
            name: '新闻分类'
        })
        //聚合查询的参数叫做聚合管道，在里面可以定义多种操作,每一个都是对象，每一个都有操作符
        const cats = await Category.aggregate([
            //第一步 执行一个match操作，条件查询，类似于数据库中的where
            //查询的条件是parent=parent._id  第一个parent是字段，第二个是我们上面查出来的顶级分类
            { $match: { parent: parent._id } },
            //第二部用lookup，类似关系型数据库里面的join
            {
                $lookup: {
                    //from表示关联哪个集合（在Article里面定义模型的时候。第三个参数省略了，他表示collection集合的名字）
                    //module.exports = mongoose.model('Article', schema,'articles'←默认是模型名的小写复数)
                    from: 'articles',
                    localField: '_id',
            ...
            //第三部，筛选查询5条数据 slice第一个参数表示要筛的字段
            {
                $addFields: {
        ...
        cats.map(cat => {
            cat.newsList.map(news => {
                //在热门那一列里面前面的【热门】固定是不对的，应该是各自对应的分类
